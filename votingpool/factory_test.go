...
func createWithdrawalTx(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, inputAmounts []int64, outputAmounts []int64) *withdrawalTx {
	...
	for i, amount := range outputAmounts {
		request := TstNewOutputRequest(
			t, uint32(i), "MTH7varKLCps8ByRGVPcEr7EvxVJ1rBjFi", ltcutil.Amount(amount), net)
...
func TstCreateSeriesCredits(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, seriesID uint32, amounts []int64) []credit {
	...
	for i := range msgTx.TxOut {
		c := wtxmgr.Credit{
			...
			Amount:   ltcutil.Amount(msgTx.TxOut[i].Value),
...
func TstNewOutputRequest(t *testing.T, transaction uint32, address string, amount ltcutil.Amount,
...
func TstConstantFee(fee ltcutil.Amount) func() ltcutil.Amount {
	return func() ltcutil.Amount { return fee }
}
...
func createAndFulfillWithdrawalRequests(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, roundID uint32) withdrawalInfo {
	...
	requests := []OutputRequest{
		TstNewOutputRequest(t, 1, "MTH7varKLCps8ByRGVPcEr7EvxVJ1rBjFi", 3e6, params),
		TstNewOutputRequest(t, 2, "MGaV6xH9kS85fUa7rTCwhPb7zaY4pAQEWS", 2e6, params),
	}
