...
func TestCreateSimpleCustomChange(t *testing.T) {
	...
	tx1, err := w.txToOutputs(
		...
		0, 1, 1000, CoinSelectionLargest, true, nil,
	)
	require.NoError(t, err)
...
func TestSelectUtxosTxoToOutpoint(t *testing.T) {
	...
	// transaction.
	// LTC: different as our coinselection is completed already unlike btc
	require.GreaterOrEqual(t, len(selectUtxos), len(tx1.Tx.TxIn),
		"transaction should use equal or fewer inputs than selected UTXOs")
